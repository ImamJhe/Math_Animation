...
class sine_curve_circle(Scene):
    def construct(self):
        ...
        # define graph
        sine = ax.plot(lambda x: np.sin(x), x_range=[0, 0], color=YELLOW)
        ...
        def update_dot(obj, dt):
            ...
            if np.radians(self.displacement * 180) <= 1080:
                obj.move_to(circle.point_from_proportion((self.displacement / 115 + 0.5) % 1))
        ...
        def update_graph(obj, dt):
            self.displacement += self.rate * dt
            if np.radians(self.displacement * 180) <= 1080:
                obj.become(ax.plot(lambda x: np.sin(np.radians(x)), x_range=[0, np.radians(self.displacement * 180)],
                                   color=YELLOW))
            else:
                self.displacement = 0  # Reset the displacement to 0 when it exceeds 360 degrees
                obj.become(ax.plot(lambda x: np.sin(np.radians(x)), x_range=[0, np.radians(self.displacement * 180)],
